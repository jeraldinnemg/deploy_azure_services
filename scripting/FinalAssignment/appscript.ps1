...
if($Action -eq "create"){
  ...
  if(!$ResourceGroup -and !$NetworkSecurityGroup -and !$VirtualNetwork -and !$Subnet -and !$KeyVault -and !$StorageAccount -and !$AppServicePlan -and !$AzureFunctionApp){
    ...
    # Create by resource
    # Validate that a name parameter was received, that it follows the EY naming convention, or generate one and save it
    if($ResourceGroupName){
    ...
    # Validate that it has been created
    if(Validate-ResourceExists -RsgOrRsc "rsg" -ResourceName $ResourceGroupName){
      Write-LogCustom -Message "Resource group $ResourceGroupName created successfully"
    }
    else{
    ...
    # If the user do not select only rsg, create the resource(s) within the same rsg.
    if($NetworkSecurityGroup){
      # Validate that received a name as a parameter that follows the EY naming convention.
      if($NetworkSecurityGroupName){
    ...
    if ($VirtualNetwork){
      ...
      # Validate that the chosen or created name is available in Azure, or else recreating it until there is no other resource with the same name.
      while(Validate-ResourceExists -RsgOrRsc "rsc" -ResourceName $VirtualNetworkName){
        Write-LogCustom -Message "The name $VirtualNetworkName is not available in Azure"
        $VirtualNetworkName = Create-ResourceName -ResourceType "Virtual Network"
      }
...
elseif($Action -eq "delete"){
  #When using only the delete parameter, the last resource group created with this script with the -create parameter is deleted.
  #In case there is no resource, the 'else' statement informs you that there is no previously created resource and suggests other actions.
  ...
  #Using -All parameter, for instance All Resource Groups within the subscription are deleted.
  elseif ($All) {
  ...
  #In case the other parameters are added, you can choose to delete individual resources by naming them by their name.
  #Only in the case that there are two resources with the same name, it alerts you from the log and asks you to enter the Resource Group. 
